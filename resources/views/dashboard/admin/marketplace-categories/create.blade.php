...
@section('title', 'Add Marketplace Category')
...
<x-layout.page
    ...
    subtitle="Create a top-level marketplace division."
    width="full"
    :breadcrumb="[
        ['Admin', route('admin')],
...
>
    <x-dashboard.card>
        <form method="POST" action="{{ route('admin.marketplace-categories.store') }}" class="w-full space-y-4" x-data="{ submitting: false }" @submit="submitting = true">
            ...
            <x-dashboard.input label="Icon key (optional)" name="icon" :value="old('icon')" placeholder="grid" />
