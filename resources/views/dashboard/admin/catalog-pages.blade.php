        ...
        @foreach($keys as $item)
            ...
            <x-ui.card>
                ...
                <form method="POST" action="{{ route('admin.catalog-pages.upsert') }}" class="grid md:grid-cols-2 gap-3">
                    ...
                    <x-ui.input label="Hero title" name="hero_title" :value="old('hero_title', $row?->hero_title)" />
                    <x-ui.input label="Hero subtitle" name="hero_subtitle" :value="old('hero_subtitle', $row?->hero_subtitle)" />
                    <x-ui.input label="Short description" name="short_description" :value="old('short_description', $row?->short_description)" class="md:col-span-2" />
                    <x-ui.input label="Banner image path" name="banner_image" :value="old('banner_image', $row?->banner_image)" placeholder="/images/..." />
                    <x-ui.input label="Card image path" name="card_image" :value="old('card_image', $row?->card_image)" placeholder="/images/..." />
