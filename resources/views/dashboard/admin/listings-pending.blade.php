                ...
                <x-ui.card variant="solid">
                    <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 shrink-0">
                            <x-ui.button type="button" size="sm" variant="success" @click="$dispatch('open-modal', 'publish-listing-{{ $l->id }}')">Publish</x-ui.button>
                            <form method="POST" action="{{ route('admin.listings.reject', $l) }}" class="flex gap-2 items-end" x-data="{ submitting: false }" @submit="submitting = true">
                                @csrf
                                <x-ui.input name="notes" placeholder="Rejection notes (optional)" size="sm" class="max-w-xs" />
                                <x-ui.button type="submit" size="sm" variant="danger" x-bind:disabled="submitting">Reject</x-ui.button>
                            </form>
                        </div>
                    </div>
                    <x-ui.modal name="publish-listing-{{ $l->id }}" title="Publish listing?" confirm-label="Publish" :form-action="route('admin.listings.approve', $l)">
