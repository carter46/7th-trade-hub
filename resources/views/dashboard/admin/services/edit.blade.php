...
>
    <x-dashboard.card>
        <form method="POST" action="{{ route('admin.services.update', $service) }}" class="max-w-form space-y-4" x-data="{ submitting: false }" @submit="submitting = true">
            ...
            <x-dashboard.input label="Banner image path" name="banner_image" :value="old('banner_image', $service->banner_image)" />
            <x-dashboard.input label="Card image path" name="card_image" :value="old('card_image', $service->card_image)" />
