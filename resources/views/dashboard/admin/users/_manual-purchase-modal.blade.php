<div
    x-data="{
        ...
        submitManualPurchase(event) {
            if (!this.action || !this.userId || !this.productSlug || !this.variantId) {
                event.preventDefault();
                this.error = 'Select a product and plan.';
                return;
            }
            if (this.isWebsite && !this.domainFqdn.trim()) {
                event.preventDefault();
                this.error = 'Enter the customer\'s existing domain.';
                return;
            }
    ...
    x-on:close-modal.window="
        ...
        if (d === 'admin-manual-purchase' || d?.id === 'admin-manual-purchase' || d?.[0] === 'admin-manual-purchase') reset();
    "
>
    <x-dashboard.modal
        name="admin-manual-purchase"
        title="Manual purchase"
        max-width="lg"
        :show-actions="false"
    >
        <form
            id="admin-manual-purchase-form"
            method="POST"
            x-bind:action="action"
            class="space-y-4"
            x-on:submit="submitManualPurchase($event)"
        >
            @csrf
            ...
            <label class="inline-flex w-full items-center justify-between gap-3 text-sm text-text-primary">
        ...
        <x-slot:footer>
            ...
            <x-dashboard.button
                type="submit"
                form="admin-manual-purchase-form"
                x-bind:disabled="submitting || loading"
            >
                <span x-show="!submitting">Confirm purchase</span>
