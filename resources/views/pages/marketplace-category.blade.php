...
<section class="max-w-marketing mx-auto px-5 sm:px-6 pb-12 sm:pb-16 space-y-10">
    @if($productCards->isNotEmpty())
        <div class="space-y-4">
            <div>
                <h2 class="text-xl font-bold font-display">Products in {{ $content['label'] }}</h2>
                <p class="text-sm text-slate-400 mt-1">Choose a product type to browse seller listings.</p>
            </div>
    ...
    @if(! empty($content['benefits']))
        <div class="rounded-xl border border-white/10 bg-slate-900/40 p-6">
            <h2 class="text-lg font-bold font-display mb-4">Why browse here</h2>
    ...
    <div class="space-y-4">
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <h2 class="text-xl font-bold font-display">Listings in {{ $content['label'] }}</h2>
            <form method="GET" action="{{ route('marketplace.show', $category->slug) }}" class="flex flex-wrap gap-3 items-end">
