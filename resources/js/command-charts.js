/**
 * Shared Chart.js defaults for command-center charts (emerald area / donut).
 */
export function commandChartDefaults() {
    return {
        responsive: true,
...
export function mountCommandCharts(root = document) {
    ...
    root.querySelectorAll('canvas.command-chart').forEach((el) => {
        ...
        const isDonut = theme === 'donut';
        const options = commandChartDefaults();
        if (isDonut) {
            delete options.scales;
            options.cutout = '72%';
            options.plugins.legend.display = false;
        } else if (theme === 'emerald-area' && datasets[0]) {
            const ctx = el.getContext('2d');
            if (ctx) {
                const gradient = ctx.createLinearGradient(0, 0, 0, el.parentElement?.clientHeight || 240);
